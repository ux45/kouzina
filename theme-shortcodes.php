...
function woo_shortcode_post_comments ( $atts ) {
	...
	$output = sprintf('<div class="icon-comment"> </div><span class="post-comments comments">%2$s%1$s%3$s</span>', $comments, $atts['before'], $atts['after']);
...
if ($style == 'list') {
	...
	while ( $qu->have_posts() ) : 
	$qu->the_post();
	woo_get_template_part( 'content', 'recipelist' );
	endwhile; 
}
else  {
	$qu = new WP_Query( $args );
...
global $post;
   ...
   if ($taxonomy == 'ingredient') {
	...
	while ( $qu->have_posts() ) : 
	$qu->the_post();
	get_template_part( 'content', 'recipelist' );
	endwhile; 
					...
					<h2 class="title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
						...
						<div class="rating">
								<?php woo_fnc_the_recipe_rating($post->ID); ?>
								 <span><?php echo woo_fnc_get_avg_rating($post->ID); ?> of 5</span>
								<?php
...
extract( shortcode_atts( array(
      ...
      return '<div class="teaserbox" style="background:'.$background.' !important;"><div class="inner"><h2 class="highlight">' .$title. '</h2>' . do_shortcode($content) . '<br /><a class="button ' .$buttonsize. ' " style="color:' .$buttoncolor. '!important;background-color:'.$buttonbackground.' !important;" href="' .$link. '" target="' .$target. '">' .$button. '</a></div></div>';
}
...
extract( shortcode_atts( array(
      ...
      if( $twitter != '' || $facebook != '' || $skype != '' || $google != '' || $linkedin != '' || $mail != '' ){
	      ...
	      if($twitter != '') {
	    	$return2 = '<li><a href="' .$twitter. '" class="icon-twitter" target="_blank" title="Twitter"></a></li>';
	      } else{
		     $return2 = ''; 
	      }
	      
	      if($facebook != '') {
	    	$return3 = '<li><a href="' .$facebook. '" target="_blank" class="icon-facebook" title="Facebook"></a></li>';
	      } else{
		      $return3 = ''; 
	      }
     
	      if($google != '') {
	    	$return4 = '<li><a href="' .$google. '" target="_blank" class="icon-google-plus" title="Google+"></a></li>';
	      } else{
		      $return4 = ''; 
	      }
	      ...
	      if($linkedin != '') {
	    	$return5 = '<li><a href="' .$linkedin. '" target="_blank" class="icon-linkedin" title="Linkedin"></a></li>';
	      }
	      else{
		      $return5 = ''; 
	      }
	      ...
	      if($mail != '') {
	    	$return6 = '<li><a href="mailto:' .$mail. '" class="icon-envelope" title="Mail"></a></li>';
	      }
	      else{
		      $return6 = ''; 
	      }
...
function woo_bloglist_sc($atts){
	...
	if( have_posts() ) :
		...
		<div class="post-meta">
		<span class="small"><div class="icon-calendar"></div> '.do_shortcode( '[post_date]' ) .'<span class="vertical-bar">&#124; </span> '. do_shortcode( '[post_comments]' ) .'</span></div>
					</div>
					</div>';
...
function woo_blog_sc($atts){
	...
	if( have_posts() ) :
		...
		$blog_thumbnail= wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'blog-grid-thumb-sc');
			...
			<div class="post-meta">
			<span class="small"><div class="icon-calendar"></div> '.do_shortcode( '[post_date after=" |"]' ) . do_shortcode( '[post_comments]' ) .'</span></div>
...
function show_method_steps($atts, $content = null) {
	...
	{
		$i = 0;
	$method_html .=  "<ul class='metstep'>";
		while($i < $steps_count)
		{
			$method_html .= '<li><span>';
			$method_html .= ($i+1).'. </span>';
...
add_shortcode('mixed_grid', 'mixed_grid_sc');   
...
function register_button_4($buttons) {  
   array_push($buttons, "toggles", "tooltip", "list", "linked_more", "googlefont","twocol_one", "threecol_one", "threecol_two", "fourcol_one", "fourcol_three", "fivecol_one","parallax","facebooklogin", "creditcard", "mixed_grid");  
   return $buttons;  
}  

function add_plugin($plugin_array) {  
   $plugin_array['creditcard'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';	
   $plugin_array['list'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['member'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['socialmedia'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['recipe_single'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['recipe_grid'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['recipe_list'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['recipe_slider'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['product_slider'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['toggles'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['linked_more'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['googlefont'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['author'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['twocol_one'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['threecol_one'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['threecol_two'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['fourcol_one'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['fourcol_three'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['fivecol_one'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['teaser'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['teaserbox'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['callout'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['blog'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['bloglist'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['testimonial'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['parallax'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['facebooklogin'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['gap'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
   $plugin_array['mixed_grid'] = get_template_directory_uri().'/includes/tinymce/tinymce.js';
